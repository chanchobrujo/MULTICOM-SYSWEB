		...
		try {
			...
			if (this.token != null && this.user != null)
				for (let index = 0; index < authorities.length; index++)
					if (authorities[index].authority == "ROLE_ADMIN")
						authority = true;
			...
			if (this.token != null && this.user != null)
				items.push(
					...
					{
						name: "Administación de clientes",
						link: "/Clientes",
			...
			if (
				...
				items.push({
					name: "Administación de usuarios",
					link: "/Usuarios",
					icon: "mdi-account",
				});
...
function navigationPage(page) {
	window.location.href =
		"http://" + window.location.host + "/" + page;
}

export { Global, closeSession, addVarLS, navigationPage };
