<template> 
    <v-container class="fill-width" fluid v-if="($global.token == null || $global.user == null)"> 
        <form @submit.prevent="login">
            <v-card width="400" class="pa-3 mx-auto mt-9" elevation="9" v-if="!isLoading">
                <v-card-title>Inicio de sesión</v-card-title>
                <v-card-text>
                    <v-text-field v-model="email" label="Correo" prepend-icon="mdi-email" ></v-text-field> 
                    <span class="red--text">{{validation.firstError('email')}}</span>
                    <v-text-field v-model="passw" label="Contraseña" prepend-icon="mdi-lock" type="password"></v-text-field> 
                    <span class="red--text">{{validation.firstError('passw')}}</span>
                </v-card-text>
                <v-card-actions>
                    <v-btn type="submit" color="primary" class="secondary--text">
                        Ingresar
                    </v-btn> 
                    <v-btn type="submit" color="primary" text >
                        Recuperar contraseña
                    </v-btn> 
                </v-card-actions> 
                <v-alert v-model="viewalert" border="right" color="red" dense icon="mdi-account" text dismissible>
                    {{message}}
                </v-alert>
            </v-card>  
            <v-card width="400" class="pa-3 mx-auto mt-9" elevation="9" v-else>
                <Loader/>
            </v-card>
        </form>  
    </v-container> 
    
</template> 
<script src="./Login.js"></script>
